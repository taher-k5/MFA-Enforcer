...
class ChallengeController extends Controller
{
    ...
    public function actionVerify(): Response
    {
        ...
        if (!$plugin->totp->isEnrolled($user)) {
            return $this->asJson([
                'success'  => false,
                'error'    => 'Your account does not have two-factor authentication enabled. Please set it up via My Account → Password & Verification.',
                'enrolUrl' => \craft\helpers\UrlHelper::cpUrl('myaccount/password'),
            ])->setStatusCode(409);
        }
