...
class EventGuard extends Component
{
    public function registerEventListeners(): void
    {
        ...
        Event::on(Category::class, Element::EVENT_BEFORE_SAVE, function (ModelEvent $event) {
            ...
            // =====================================================
            // First category create — Never require MFA
            // =====================================================
        ...
        // ---- Craft 5: delete-elements/delete (new element deletion endpoint) ----
        // DeleteElementsController::actionDelete() calls deleteElement() for each element
        // but does NOT check its return value, so when EVENT_BEFORE_DELETE sets
        // isValid=false the delete is silently skipped while the controller still returns
        // an empty 200 response. We intercept at ACTION level to return a proper 400
        // error so the browser's JS can display the MFA challenge (and so the PHP-side
        // EVENT_BEFORE_DELETE handlers are backed up by an explicit gate).
        Event::on(Application::class, Application::EVENT_BEFORE_ACTION, function (ActionEvent $event) {
            try {
                $request = Craft::$app->getRequest();
                ...
                $actionId = (string)($event->action->getUniqueId() ?? '');
                if (stripos($actionId, 'delete-elements/delete') !== 0) {
                    return;
                }

                $elementType = (string)($request->getBodyParam('elementType', ''));
                $elementIds  = array_filter(array_map('intval', (array)($request->getBodyParam('elementIds', []))));

                if (empty($elementType) || empty($elementIds)) {
                    return;
                }
                ...
                if (is_a($elementType, Entry::class, true)) {
                    ...
                    foreach ($sectionIds as $sectionId) {
                        if ($settings->isScopedActionProtected('entry', (int)$sectionId, 'delete')) {
                            $needsMfa = true;
                            break;
                        }
                    }
                } elseif (is_a($elementType, Category::class, true)) {
                    $groupIds = (new Query())
                        ->select(['groupId'])
                    ...
                    foreach ($groupIds as $groupId) {
                        if ($settings->isScopedActionProtected('category', (int)$groupId, 'delete')) {
                            $needsMfa = true;
                            break;
                        }
                    }
                } elseif (is_a($elementType, Asset::class, true)) {
                    $volumeIds = (new Query())
                        ->select(['volumeId'])
                        ->from([Table::ASSETS])
                        ->where(['id' => $elementIds])
                    ...
                    foreach ($volumeIds as $volumeId) {
                        if ($settings->isScopedActionProtected('asset', (int)$volumeId, 'delete')) {
                            $needsMfa = true;
                            break;
                        }
                    }
    ...
    private function verifyOrReasonForUpload(): ?string
    {
        ...
        if (!$plugin->totp->isEnrolled($user)) {
            return 'This action requires two-factor authentication, but your account does not have it enabled. Please set it up via My Account → Password & Verification before retrying.';
        }
